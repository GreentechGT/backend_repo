...
class MainOrderListView(generics.ListAPIView):
    """
    ...
    def get_queryset(self):
        return MainOrder.objects.filter(user=self.request.user).order_by('-created_at')
...
class VendorPaymentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        vendor_id = self.request.user.user_id
        if not vendor_id:
            return VendorSettlement.objects.none()
        return (
...
class VendorDashboardView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer_context = {'vendor_id': vendor_id}
